...
async function initPage() {
  ...
  initNavigation("view-commands.html", authStatus);
}
...
async function technologyFilter() {
  try {
    const technologies = await loadCategories();
...
function applyFilter() {
  ...
  if (selectedTechId) {
    filteredCommands = filteredCommands.filter(cmd =>
      cmd.category_id == selectedTechId || cmd.category_id === parseInt(selectedTechId)
    );
  }
...
async function deleteCommand(commandId) {
  try {
    const response = await fetch(`/api/commands/move-to-trash/${commandId}`, {
      method: 'POST'
    });
    const result = await response.json();
    if (result.success) {
...
async function saveCommand(commandId) {
  ...
  try {
    const response = await fetch(`/api/commands/update/${commandId}`, {
      method: "POST",
      headers: {
    ...
    if (result.success) {
      loadCommands();
      showFeedback({ success: true, message: `${window.i18n.translate("pages.viewCommands.cmdCard.updated")}` });
    }
  } catch (error) {
    showFeedback({ success: false, message: `${window.i18n.translate("pages.viewCommands.cmdCard.updateError")}` });
  }
}
